...
def main():
    ...
    data_arr_01 = data_loader.load_data(r'proj/data.txt')
    label_vec = data_loader.load_label(r'proj/label.txt')
    ...
    ## make models
    model = dcnn.Net('DCNN19', 1, 5)

    ## train
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), weight_decay=0.0001)
    best_model, loss_curve = iter_utils.train(model, train_loader, criterion, optimizer,
        init_lr=0.0001, decay_epoch=5, n_epoch=100, use_cuda=True)

    # save model
    torch.save(best_model.state_dict(), "data/dcnn19.pth")
